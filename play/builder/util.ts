import type { PageContext, PageMetaDatum } from '@uni-helper/vite-plugin-uni-pages'
...
export function getServerProxy(list: [string, string][]): Record<string, string | ProxyOptions> {
...
export function scanPageFilter(ctx: PageContext, inKey: 'pages' | 'subPages'): void {
  // 页面文件结尾
...
export function handlePageName(ctx: PageContext, inKey: 'pageMetaData' | 'subPageMetaData'): void {
  const nameRegExp = /[/.-]/g
...
export function writePageConst(ctx: PageContext): void {
  const spinner = ora({
  ...
  try {
    ...
    const lintResult = eslint.verifyAndFix(fileContent, eslintConfig as unknown as Linter.Config[], filepath)
    const formattedContent = lintResult.output
